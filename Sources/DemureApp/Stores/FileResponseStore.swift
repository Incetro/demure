...
// MARK: - FileResponseStore

final class FileResponseStore {
    ...
    private func filePath(for request: Request) -> String {
        directory.absoluteString + request.url.path
    }
...
extension FileResponseStore: ResponseStore {
    ...
    func perform(_ action: DemureAction, for request: Request) -> EventLoopFuture<Response> {
        ...
        return writeFile(data: body, at: path, request: request).map { _ in
